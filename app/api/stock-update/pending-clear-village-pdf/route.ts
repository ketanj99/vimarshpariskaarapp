...
import {
  getPendingClearVillageData,
  generatePendingClearVillagePdf,
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rows = await getPendingClearVillageData(pNos, app, groupSector);
    if (rows.length === 0) {
      return NextResponse.json(
        { error: 'No village-wise data found for the given P No. filter.' },
        { status: 404 }
      );
    }
    const pdfBuffer = generatePendingClearVillagePdf(rows);
    const filename = `PendingClear_VillageWise_${new Date().toISOString().slice(0, 19).replace(/[-:T]/g, '')}.pdf`;
    ...
    console.error('Pending clear village PDF error:', error);
    return NextResponse.json(
